...
- Disagreements / to verify: ⚠️ **CORRECTED after live re-test (Ayselin, August 2026):** Earlier live testing (via the "In a hurry?" quick-contact form) found no figure was ever shown, appearing to confirm the Round 1/4 finding. However, completing the full main form end-to-end (postcode → address selection → bedrooms → property type) DOES produce a real results screen: Sales Valuation with Minimum/Average/Maximum figures (e.g. £174,000 / £237,287 / £301,000), a Lettings Valuation range, and an interactive "What do you think it's worth?" slider. **This overturns the original Round 1 and Round 4 claim that the tool never displays a figure.** The correct finding is: the outcome depends on which path the user takes — the "In a hurry?" shortcut form skips straight to lead capture with no figure, while the full step-by-step flow does surface a real instant valuation range. This is a UX/messaging issue worth flagging on its own (two entry points on the same page produce very different experiences), separate from the original AI-explanation-layer opportunity, which still applies: even the real figures shown here come with no explanation of what drove them.
...
- Öne çıkan fırsatlar: (1) "Heads-Up Alerts" gerçek anlamda akıllı hale getirilebilir. (2) "100% MATCH" rozetinin gerçek bir eşleştirme motoruna bağlanması (sahte kişiselleştirme, güven riski).
- Görüş ayrılıkları / doğrulanması gerekenler: ⚠️ **Canlı yeniden testten sonra DÜZELTİLDİ (Ayselin, Ağustos 2026):** Önceki canlı test ("In a hurry?" hızlı iletişim formu üzerinden) hiç rakam gösterilmediğini bulmuştu, bu da Round 1/4 bulgusunu doğrular gibi görünüyordu. Ancak ana formu baştan sona (postcode → adres seçimi → yatak odası → mülk tipi) tamamlayınca **gerçek bir sonuç ekranı çıkıyor**: Sales Valuation için Minimum/Average/Maximum rakamlar (örn. £174,000 / £237,287 / £301,000), Lettings Valuation aralığı, ve interaktif bir "What do you think it's worth?" kaydırıcısı. **Bu, Round 1 ve Round 4'teki "araç asla rakam göstermiyor" iddiasını çürütüyor.** Doğru bulgu şu: sonuç, kullanıcının hangi yolu seçtiğine bağlı — "In a hurry?" kısayolu doğrudan lead-capture'a gidiyor ve rakam göstermiyor, ama tam adım adım akış gerçek bir anlık değerleme aralığı gösteriyor. Bu, aynı sayfada iki farklı giriş noktasının çok farklı deneyimler üretmesi anlamında kendi başına bir UX/mesajlaşma sorunu olarak işaretlenmeye değer — orijinal AI-açıklama-katmanı fırsatı hâlâ geçerli: burada gösterilen gerçek rakamlar bile neye dayandığına dair hiçbir açıklama içermiyor.
