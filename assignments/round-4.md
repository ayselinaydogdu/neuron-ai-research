# Round 4 — Site Assignments / Tur 4 — Site Atamaları

## English

This is **copy 4** of the assignment rotation. Each round everyone gets 5–6 sites.
After 4 rounds everyone sees every group → each site is reviewed 4 times, by 4 different people.

| Person | Group | Sites |
|---|---|---|
| Ayselin | A | 01–05 |
| Yasemin | B | 06–10 |
| Berkay | C | 11–15 |
| Görkem | D | 16–21 |

**How to use:** open the sites assigned to you below one by one, fill in the
**Round 4** block in each site file, then tick the box here ([x]) and commit.
For the full rotation see ../site-assignments.md.

### Ayselin — Group A (01–05)

- [ ] 01 · kineticestateagents.co.uk → ../sites/01-kinetic.md
- [ ] 02 · townandcityhomes.com → ../sites/02-townandcity.md
- [ ] 03 · harrisonshomes.co.uk → ../sites/03-harrisons.md
- [ ] 04 · lloydsestates.com → ../sites/04-lloyds.md
- [ ] 05 · thepropertycloud.uk → ../sites/05-propertycloud.md

### Yasemin — Group B (06–10)

- [ ] 06 · propertyconnections.uk.com → ../sites/06-propertyconnections.md
- [ ] 07 · keystoneestateagents.co.uk → ../sites/07-keystone.md
- [ ] 08 · jacksonsproperty.co.uk → ../sites/08-jacksons.md
- [ ] 09 · harrisandwood.co.uk → ../sites/09-harrisandwood.md
- [ ] 10 · roderickthomas.co.uk → ../sites/10-roderickthomas.md

### Berkay — Group C (11–15)

- [ ] 11 · copeandco.co.uk → ../sites/11-copeandco.md
- [ ] 12 · priceestateagents.uk → ../sites/12-price.md
- [ ] 13 · keysandlee.co.uk → ../sites/13-keysandlee.md
- [ ] 14 · nicholsonsestateagents.co.uk → ../sites/14-nicholsons.md
- [ ] 15 · no86estateagency.co.uk → ../sites/15-no86.md

### Görkem — Group D (16–21)

- [x] 16 · beercocks.com → ../sites/16-beercocks.md
- [x] 17 · greyandco.co.uk → ../sites/17-greyandco.md
- [x] 18 · allen-residential.co.uk → ../sites/18-allen.md
- [x] 19 · brownsestateagents.co.uk → ../sites/19-browns.md
- [x] 20 · bydesignhomes.com → ../sites/20-bydesign.md
- [x] 21 · courtyardhomes.co.uk → ../sites/21-courtyard.md

---

## Türkçe

Bu, atama rotasyonunun **4. kopyasıdır**. Her tur herkese 5–6 site düşer.
4 tur sonunda herkes tüm grupları görür → her site 4 kez, 4 farklı kişi tarafından incelenmiş olur.

| Kişi | Grup | Siteler |
|---|---|---|
| Ayselin | A | 01–05 |
| Yasemin | B | 06–10 |
| Berkay | C | 11–15 |
| Görkem | D | 16–21 |

**Nasıl kullanılır:** Aşağıda size düşen siteleri tek tek açın, ilgili site
dosyasında **Tur 4** bloğunu doldurun, bitirince buradaki kutucuğu işaretleyin
([x]) ve commit'leyin. Rotasyonun tamamı için ../site-assignments.md.

### Ayselin — Grup A (01–05)

- [ ] 01 · kineticestateagents.co.uk → ../sites/01-kinetic.md
- [ ] 02 · townandcityhomes.com → ../sites/02-townandcity.md
- [ ] 03 · harrisonshomes.co.uk → ../sites/03-harrisons.md
- [ ] 04 · lloydsestates.com → ../sites/04-lloyds.md
- [ ] 05 · thepropertycloud.uk → ../sites/05-propertycloud.md

### Yasemin — Grup B (06–10)

- [ ] 06 · propertyconnections.uk.com → ../sites/06-propertyconnections.md
- [ ] 07 · keystoneestateagents.co.uk → ../sites/07-keystone.md
- [ ] 08 · jacksonsproperty.co.uk → ../sites/08-jacksons.md
- [ ] 09 · harrisandwood.co.uk → ../sites/09-harrisandwood.md
- [ ] 10 · roderickthomas.co.uk → ../sites/10-roderickthomas.md

### Berkay — Grup C (11–15)

- [ ] 11 · copeandco.co.uk → ../sites/11-copeandco.md
- [ ] 12 · priceestateagents.uk → ../sites/12-price.md
- [ ] 13 · keysandlee.co.uk → ../sites/13-keysandlee.md
- [ ] 14 · nicholsonsestateagents.co.uk → ../sites/14-nicholsons.md
- [ ] 15 · no86estateagency.co.uk → ../sites/15-no86.md

### Görkem — Grup D (16–21)

- [x] 16 · beercocks.com → ../sites/16-beercocks.md
- [x] 17 · greyandco.co.uk → ../sites/17-greyandco.md
- [x] 18 · allen-residential.co.uk → ../sites/18-allen.md
- [x] 19 · brownsestateagents.co.uk → ../sites/19-browns.md
- [x] 20 · bydesignhomes.com → ../sites/20-bydesign.md
- [x] 21 · courtyardhomes.co.uk → ../sites/21-courtyard.md
