...
- Commonly missing AI features: Natural-language search, AI listing summaries, Property Q&A, personalised similar-listing recommendations, AI-powered area/commute insights, EPC/floor plan explanation, AI lead scoring, out-of-hours automated response, instant valuation, and mortgage/stamp-duty calculators.

- Standout opportunities: The strongest finding is that Chatbot, InstantValuation and StampDutyCalculator components are present in the site's source but inactive, suggesting that useful platform capabilities already exist without being enabled for this tenant. The site also has rich structured and editorial data that could support AI features: Heads Up collects moving timeframe, solicitor status, need-to-sell and mortgage/cash information, while listings contain detailed "Seller's Story" and "Location" content that could directly support grounded AI summaries, Property Q&A and lead qualification. Natural-language search would also be a logical improvement, particularly on mobile where the existing structured filters are compressed into dropdown panels.

- Disagreements / to verify: The "100% MATCH" feature needs clarification. Round 1 describes it as a prediction based on the user's unique saved-search criteria, while later rounds observed every listing showing 100% even when no criteria had been entered, suggesting that the visible score is not meaningfully personalised in its default state. Instant valuation is consistently confirmed as unavailable across all rounds, despite an InstantValuation component being present in the source; this appears to be a tenant configuration or deliberate product choice rather than a missing platform capability. No separate AI/LLM service call was observed in the technical trace; third-party/platform services identified include Neuron REST, IcebergTracker and Sentry, while Chatbot, InstantValuation and StampDutyCalculator components remain inactive.
...
- Ortak eksik AI özellikleri: Doğal dil arama, AI ilan özeti, Property Q&A, kişiselleştirilmiş benzer ilan önerileri, AI destekli alan/commute analizi, EPC/floor plan açıklaması, AI lead scoring, mesai dışı otomatik yanıt, instant valuation ve mortgage/stamp-duty hesaplayıcıları.

- Öne çıkan fırsatlar: En güçlü bulgu, Chatbot, InstantValuation ve StampDutyCalculator bileşenlerinin sitenin kaynak kodunda bulunmasına rağmen aktif olmaması; yani yararlı platform özelliklerinin altyapısı mevcut ancak bu tenant için devreye alınmamış görünüyor. Site aynı zamanda AI özellikleri için kullanılabilecek zengin yapılandırılmış ve editoryal veriye sahip: Heads Up taşınma zamanı, solicitor durumu, mevcut mülkü satma ihtiyacı ve mortgage/nakit durumu gibi bilgiler toplarken ilanlardaki ayrıntılı "Seller's Story" ve "Location" içerikleri AI ilan özeti, Property Q&A ve lead qualification için doğrudan kaynak oluşturabilir. Özellikle mobilde mevcut filtrelerin açılır panellere sıkışması nedeniyle doğal dil arama da mantıklı bir geliştirme alanı.

- Görüş ayrılıkları / doğrulanması gerekenler: "100% MATCH" özelliğinin çalışma şekli netleştirilmeli. Round 1 bunu kullanıcının benzersiz kayıtlı arama kriterlerine dayanan bir tahmin olarak değerlendirirken sonraki turlarda hiçbir kriter girilmediğinde dahi tüm ilanların %100 gösterdiği gözlemlendi; bu nedenle görünür skorun varsayılan durumda anlamlı biçimde kişiselleştirilmediği görülüyor. Instant valuation'ın bulunmadığı tüm turlarda tutarlı biçimde doğrulandı; buna rağmen kaynakta InstantValuation bileşeninin bulunması bunun platform eksikliğinden ziyade tenant yapılandırması veya bilinçli bir ürün tercihi olduğunu düşündürüyor. Teknik incelemede ayrı bir AI/LLM servis çağrısı gözlemlenmedi; tespit edilen üçüncü taraf/platform servisleri arasında Neuron REST, IcebergTracker ve Sentry bulunurken Chatbot, InstantValuation ve StampDutyCalculator bileşenleri pasif durumda.
