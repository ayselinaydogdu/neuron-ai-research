...
- Görüş ayrılıkları / doğrulananlar: ✅ **Yasemin tarafından doğrulandı (canlı kontrol, 17 Ağustos 2026):** Network sekmesinde ayrı bir Neuron content-API çağrısı **olmadığı doğrulandı** — 5/120 request Fetch/XHR ve hepsi tracking: `ice.js/anonymous`, 3× Sentry envelope, bir cached UUID (`f5568c46-…`). `filter` çağrısı yok. Bu, 11 · Cope&Co gözlemiyle tutarlı. **Build farkı da doğrulandı:** Sentry release hash'i `9ce27bec7a14fa785eb9fd8d36c531fc71808b27` ve görsel pipeline `/_ipx/` — yani 08 · Jacksons ve 11 · Cope&Co ile aynı grupta. 09 · Harris + Wood ve 10 · Roderick Thomas ise `fa3a1c21fbc31083c37fb3f41928c0c10c23476f` + `/_vercel/image` kullanıyor. Üç sitede tekrarlandığı için bu gerçek bir platform/build ayrımı. **Ancak:** 09 ve 10'un arama sayfalarında da ayrı bir content-API `filter` çağrısı gözlenmedi; `filter` çağrısının hangi sitede gözlendiği teyit edilmeli. **Release hash zamanla değişir, ölçüm 17 Ağustos 2026 tarihlidir.**

**Uyuyan bileşenler doğrulandı:** Konsolda `document.querySelector('.whatsapp-chatbot')` ve `document.querySelector('[class*="BlendedFee"]')` ikisi de `null` dönüyor — bileşenler kaynakta referanslanıyor ama DOM'a hiç basılmıyor. 08 · Jacksons ve 10 · Roderick Thomas'ta da aynı sonuç alındı; "platform seviyesinde altyapı var ama hiçbir tenant'ta açık değil" örüntüsü üç siteyle doğrulanmış oldu.