...
- Görüş ayrılıkları / doğrulananlar: ✅ **Yasemin tarafından doğrulandı (canlı kontrol, 17 Ağustos 2026):** **(1) Viewing formu iddiası doğrulandı.** Takvim + saat seçici ve "This does not confirm the viewing" uyarısı 09 · Harris + Wood ve 10 · Roderick Thomas'ta da birebir aynı şekilde bulundu. Üç sitede aynı bileşen, aynı metin — platform genelinde bir güven/beklenti uyuşmazlığı, siteye özgü değil. **(2) ÖNEMLİ DÜZELTME — `filter` çağrısı build farkından kaynaklanmıyor.** Keys & Lee'nin Sentry release hash'i `9ce27bec7a14fa785eb9fd8d36c531fc71808b27` ve görsel pipeline `/_ipx/` — yani 11 · Cope&Co ve 12 · Price ile **birebir aynı build**. Buna rağmen `filter` çağrısı canlı olarak doğrulandı: `/sales/property-for-sale?is_available=true&sort=suggested` sayfasında Fetch/XHR'da **iki adet** `filter?search_type=sales&is_website…` çağrısı görülüyor (200, 31.7 kB / 326 ms ve 34.1 kB / 349 ms, ikisi de `DGx-v1bZ.js:20` üzerinden). 11 · Cope&Co ve 12 · Price'ta ise XHR listesinde yalnızca tracking çağrıları vardı. Dolayısıyla Cope&Co ve Price notlarında öne sürülen "build farkı bu davranışı açıklıyor" hipotezi **elenmiştir**; fark tenant seviyesinde bir config/veri farkı, nedeni araştırılmalı. **Release hash zamanla değişir, ölçüm 17 Ağustos 2026 tarihlidir.**
