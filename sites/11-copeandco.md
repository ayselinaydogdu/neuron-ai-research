...
- Görüş ayrılıkları / doğrulananlar: ✅ **Yasemin tarafından doğrulandı (canlı kontrol, 17 Ağustos 2026):** **(1) Build farkı gerçek.** Sentry release hash'leri iki ayrı grup gösteriyor: 11 · Cope&Co ve 08 · Jacksons `9ce27bec7a14fa785eb9fd8d36c531fc71808b27`, 09 · Harris + Wood ve 10 · Roderick Thomas ise `fa3a1c21fbc31083c37fb3f41928c0c10c23476f`. Fark sürüm numarasıyla sınırlı değil — görsel optimizasyon pipeline'ı da farklı (`/_ipx/` vs `/_vercel/image`). Yani Round 3 incelemecisinin Network'te farklı bir davranış görmesi "tek seferlik gözlem eksikliği" değil, gerçek bir platform/build farkına denk geliyor. **Not:** 09 ve 10'un arama sayfalarında da ayrı bir content-API `filter` çağrısı gözlenmedi (yalnızca `ice.js/anonymous`, Sentry envelope'ları, `view`); veri SSR ile geliyor. `filter` çağrısının hangi sitede gözlendiği teyit edilmeli. **Release hash zamanla değişir, bu ölçüm 17 Ağustos 2026 tarihlidir.** [(2) ve (3) canlı test sonuçları buraya]

**Ek bulgu — dürüst doğruluk beyanı:** Cope&Co instant valuation'ını açıkça **"60% – 70% ACCURATE"** diye etiketliyor ve *"The characteristics of the property aren't taken into account"* diyor. İncelenen siteler içinde bir değerleme aracının doğruluk oranını sayısal olarak beyan ettiği tek örnek. Araç yine ayrı subdomain'de (`valuation.copeandco.co.uk/home/4651`) — 08 · Jacksons (`/home/2488`) ve 10 · Roderick Thomas (`/home/4910`) ile aynı desen.